...
import { useEnterNewPassword, useForgotPassword } from 'src/api/employees/Mutations';

export default function EnterNewPassword() {
  const { id } = useParams();
  console.log(id);
  const enterNewPasswordMutation = useEnterNewPassword();
  ...
  const onSubmit = (data) => {
    ...
    enterNewPasswordMutation.mutate(values);
    console.log(data);
  };
    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Grid paddingBottom={2} container spacing={2} alignItems="center">
        <>
          ...
          <CustomSubmitButton
            disabled={enterNewPasswordMutation.isPending}
            name={enterNewPasswordMutation.isPending ? 'Loading' : `Reset Password`}
          />{' '}
        </>
