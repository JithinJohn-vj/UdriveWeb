...
import { useGetCarsRevenueById } from 'src/api/cars/Queries';
import { fShortenNumber } from 'src/utils/formatNumber';
import { useGetCarssById } from 'src/api/calendar/Queries';

export default function CarsDetailsModal({ open, onClose, userDetails }) {
  ...
  const singleCarRevenue = useGetCarsRevenueById(userDetails?.id);
  const kmsCovered = useGetCarssById();
  console.log(userDetails);
  return (
    <Container>
      <Dialog maxWidth="lg" fullWidth={true} open={open} onClose={onClose}>
        <DialogTitle className="bg-[#59c5b9] text-white  rounded">Car Detailed view</DialogTitle>
        <IconButton
          aria-label="close"
          onClick={onClose}
          sx={{
              ...
              <Card className="p-3 shadow-xl border border-black border-dotted border-opacity-20">
                <Grid container spacing={3} sx={{ height: { xs: 'auto', md: 180 } }}>
                  <Grid className="flex justify-center items-center" item xs={12} md={3}>
                    <Avatar
                      variant="square"
                      style={{ width: '140px', height: '110px', cursor: 'pointer' }}
                      src={userDetails?.carImage}
                    />
                  </Grid>
                  <Grid
                    item
                  ...
                  >
                    {/* transmission: 'manual',
    service_interval: 'Every 3 Months',
    company: 'Audi',
    fuel: 'petrol',
    year: 2018,
    insurance: '2024-04-16T18:30:00.000Z',
    lastService: '2024-04-22T18:30:00.000Z', */}
                    <div className="flex flex-col justify-around h-full">
                      ...
                      <Typography className="text-lg mt-2" variant="subtitle1">
                        {userDetails?.vehicleNumber} | Year :{userDetails?.year}
                      </Typography>
                      <Typography className="text-lg mt-2" variant="subtitle1">
                        Transmission :{userDetails?.transmission}
                      </Typography>
                    </div>
                  </Grid>
                  <Grid item xs={12} md={4} justify="center" alignItems="center">
                    <div className="flex flex-col justify-around h-full">
                      ...
                      <Typography className="text-2xl mt-2" variant="overline">
                        Insurance - {CustomformatDate(userDetails?.insurance)}
                      </Typography>
                      <Typography className="text-lg mt-2" variant="subtitle1">
                        last Service - {CustomformatDate(userDetails?.lastService)}
                      </Typography>
                      <Typography className="text-lg mt-2" variant="subtitle1">
                        Service on : {userDetails?.service_interval},
                      </Typography>
                    </div>
                  </Grid>
                </Grid>
                  ...
                  <div className="flex flex-col justify-around h-full w-full text-center">
                    ...
                    <Typography className="text-2xl mt-2 text-white" variant="overline">
                      <span className="text-2xl">
                        ₹ {fShortenNumber(singleCarRevenue?.data || 0)}
                      </span>
                    </Typography>
                    <Typography
                      variant="body2"
                      className="text-2xl    -full font-extrabold text-white text-left px-3"
                    >
                      <span className="text-">Kms covered</span>
                      <br />
                      <span className="text-">{userDetails?.totalKmCovered || 0}</span>
                    </Typography>
                  </div>
                </Grid>
              </Card>
                        ...
                        <>
                          ...
                          >
                            ...
                            <div className="grid md:grid-cols-3 mt-2 space-y-2 pb-4">
                              <Typography className="text-2xl" variant="subtitle2">
                                Name: <span className="text-lg">{m.customerSelected.name}</span>
                              </Typography>
                              <Typography className="text-2xl" variant="subtitle2">
                                Contact Number :{' '}
                                <span className="text-lg">{m.customerSelected.contactNumber}</span>
                              </Typography>
                              <Typography className="text-2xl" variant="subtitle2">
                                Email : <span className="text-lg">{m.customerSelected.email}</span>
                              </Typography>

                              <Typography className="text-2xl" variant="subtitle2">
                                Passport Number :{' '}
                                <span className="text-lg">{m.customerSelected.passportNumber}</span>
                              </Typography>
                              <Typography className="text-2xl" variant="subtitle2">
                                Pincode :{' '}
                              ...
                              <Typography className="text-2xl" variant="subtitle2">
                                State : <span className="text-lg">{m.customerSelected.state}</span>
                              </Typography>
                              <Typography
                              ...
                              <Typography className="text-2xl" variant="subtitle2">
                                City/District :{' '}
                                <span className="text-lg">{m.customerSelected.cityOrDistrict}</span>
                              </Typography>
                            </div>
                          </Grid>
